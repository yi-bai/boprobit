...
# @the order of stval should be: c(alpha1, alpha2, alpha3, b, c1, c2, lamda, iota, gam1, gam2)
st_alpha <- polr(as.factor(inj) ~ x1 + x2 + fatig, data=mydata, method = "probit")
summary(st_alpha)

st_fatig <- glm(fatig ~ z1 + z2, family=binomial(link="probit"), data=mydata)
...
# use optim to do MLE
result1 <- optim(stval, GBOP, dat = mydata, method="BFGS", hessian = TRUE) # Estimate the model
print(result1)
